    ...
    *command = command
    ...
    let (requires, approval, time, prints, examples) = match key {
        ...
        "data list" => (
            ...
            "seconds",
            "slot, state, origin, build, version, size, requires; JSON adds id, name, description, format, damage, download",
            "ark data list\nark data list --json",
        ),
        "data show" => (
            "a paired, unlocked Ark (--unlock only if status reports locked)",
            "none; --unlock needs your phone",
            "seconds",
            "slot, id, name, description, format, state, origin, damage, dependencies, size, build, version, download, required_by, cached; JSON uses requires, size_bytes",

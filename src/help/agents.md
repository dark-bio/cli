...
- Nothing prompts when stdin is not a terminal, under --no-input, or in JSON
  mode. --yes confirms firmware installation. The owner still approves
  operations on the Ark when required.
- Every command blocks until done and its exit code is the outcome. Run long
  ...
  Expect seconds for status, up to a minute for an approval, minutes for an
  app run, minutes to an hour for an upload, and an hour or more for a reference
  catalog. --timeout bounds each machine wait, never a person or a total.
  A repeated processing percentage counts as a reply. Use your shell's timeout
  utility for a workflow ceiling. Ctrl-C and SIGTERM attempt cancellation;
  the task id printed by app run also works with app cancel.
...
CLI error codes are stable. error[ark]: passes through the Ark's own verdict;
read its message, never match its number or wording. Partial results survive
errors and interruption. Text field names match the JSON keys.
...
ark unlock, and lasts until power is cut. Data and app commands need an
unlocked Ark. Pass --unlock to allow unlocking first; a dry run requires

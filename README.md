...
trust policy and transport adapters; `darkbio-wire` owns the encrypted protocol,
multiplexing, worker threads and session lifecycle. `list()` discovers both
...
for a budget starting at that call. Request deadlines cover queueing, sending
and accepting the response. Discovery, connection setup and response decoding
are outside that budget. Timing out or dropping a pending result does not
...
the CLI currently exposes status and onboarding.
...
`Client::send` uses wire's unbounded output queue; callers manage the number
of outstanding requests. `Pending::notify` lets one channel observe many
completions without one waiting thread per request.
